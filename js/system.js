...
var url=window.location.href;if(url.indexOf("?")!=-1) {url=url.split("?");window.location.href =url[0];} else{} 
var MAC={
    'Url': document.URL,
    ...
    'Hits': {
        'Init':function() {
            ...
            var nid=$(".mac_hits").attr("data-nid");
            var $that = $(".mac_hits");
            if(!nid){
                url=maccms.path + '/index.php/ajax/hits?mid='+$that.attr("data-mid")+'&id='+$that.attr("data-id")+'&type=update';
            }else{
                url=maccms.path + '/index.php/ajax/hits?mid='+$that.attr("data-mid")+'&id='+$that.attr("data-id")+'&type=update&dm='+nid;
